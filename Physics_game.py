...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if generate_particle_negative_button.left <= mouse[0] <= generate_particle_negative_button.right and generate_particle_negative_button.top<=mouse[1] <= generate_particle_negative_button.bottom:
                new_particle = Physics_utils.Particle(box,0,1,5,-5)
                particles.append(new_particle)
        ...
        if event.type == pygame.KEYDOWN: 
            if mass_active ==True:
                ...
                if len(mass_text) ==0 and event.key == pygame.K_MINUS:
                    mass_text += '-'
    ...
    generate_particle_positve_button = pygame.draw.rect(screen,color=(255,0,0),rect=pygame.Rect(box_2.left+4,box_2.bottom-104,100,100))
    ...
    generate_particle_negative_button = pygame.draw.rect(screen,color=(0,0,255),rect=pygame.Rect(generate_particle_positve_button.right+4,box_2.bottom-104,100,100))
    screen.blit(font.render("Generate",True,(255,255,255)),(generate_particle_negative_button.left,generate_particle_negative_button.top+20))
    ...
    for i,p in enumerate(particles):
        ...
        if len(particles) >1:
            for j in range(i+1,len(particles)):
                ...
                if p.charge * particles[j].charge <= 0:
